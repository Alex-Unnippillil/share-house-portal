...
interface MaintenanceOverviewCardClientProps {
  tickets: MaintenanceTicket[]
}

export function MaintenanceOverviewCardClient({ tickets }: MaintenanceOverviewCardClientProps) {
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <ul className="space-y-3">
          {tickets.map((ticket) => (
            <li key={ticket.id} className="rounded-lg border border-dashed border-border/70 p-3">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="space-y-1">
                  <p className="text-sm font-medium text-foreground">{ticket.title}</p>
                  <p className="text-xs text-muted-foreground">
                    Last updated {formatDate(ticket.updatedAt, { month: "short", day: "numeric" })}
                  </p>
                </div>
