...
The app automatically derives the Supabase REST endpoint from `DATABASE_URL`. You can still override it explicitly with `NEXT_PUBLIC_SUPABASE_URL` (or `SUPABASE_URL` on the server) if needed.
...
4. Copy the Project `DATABASE_URL` from the `Project Settings → Database` page
5. Copy the anon/public key (or use the Supabase CLI output)
6. Copy the service_role key (keep this secret!)
7. Get JWT secret from Settings > API > JWT Settings

### **Supabase configuration precedence**

Supabase credentials are resolved with the following priority:
...
This means you can launch the app locally by providing just the `DATABASE_URL`, anon key, and service role key generated by `supabase start` without duplicating the REST URL.
