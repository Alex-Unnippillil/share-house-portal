...
Use the `CountryDetailSkeleton` for country-specific pages and Suspense boundaries:
...
The skeleton advertises loading state through `aria-busy` and `role="status"`, and mirrors the primary page hierarchy with a hero heading placeholder, metadata pills, and grid cards.
