...
| Payments | Rent payment webhooks persist receipts and trigger tenant comms | Existing payment domain tests in `tests/payments-status.test.ts` and `tests/catch-up.test.ts`. | `tests/integration/api/stripe-webhook.test.ts` validates Stripe webhook handling, Supabase writes, notification fanout, and invoice failure payload drift handling. | `e2e/release-regression-negative.spec.ts` validates malformed checkout requests are rejected. | API error assertions + smoke checks under `pnpm test:unit`; performance budget includes `pnpm css:purge`. |
...
| Bookings | Tenant can reserve amenities with conflict-safe event type selection | Booking logic path verified via service behavior tests. | `tests/integration/services/calcom-service.test.ts` validates booking payload shape + no-slot fallback messaging, including event payload drift. | `e2e/release-regression-negative.spec.ts` validates invalid booking windows are blocked. | Landmark + heading checks in booking journey; CSS size guard in perf script. |
...
| Documents | Tenants can receive signing URLs from Documenso-backed flows | Document model tests in `tests/lib/data/documents.test.ts`. | `tests/integration/services/documenso-service.test.ts` validates envelope creation, upload errors, and signing URL contract drift fallback. | Covered through navigation smoke in tenant journey discovery. | Existing accessibility checks cover landmark + heading consistency for core app shells. |
| RBAC/Authorization | Non-privileged users cannot reach privileged APIs | `tests/auth-rbac.test.ts` validates route/role guard behavior. | Existing API authorization tests and role helpers. | `e2e/release-regression-negative.spec.ts` validates unauthenticated export requests receive `401 Unauthorized`. | N/A |

## Mandatory Regression Paths by Role

### Tenant
- Dashboard and onboarding visibility.
- Payments entry + failed payment handling.
- Booking validation for valid and invalid time windows.
- Visitor booking and messaging/document discovery.

### Property manager
- Maintenance triage surface.
- Visitor oversight and booking conflict visibility.
- Export endpoints required for operational workflows.

### Admin
- Operations dashboard and privileged analytics surfaces.
- Reconciliation/exports and webhook-powered monitoring endpoints.
- RBAC rejection for unauthenticated or non-privileged access.
