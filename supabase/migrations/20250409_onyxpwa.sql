...
create table public.profiles (
  ...
  role text null default 'resident'::text,
...
alter table public.profiles
  ...
  check (role in ('resident','house_manager','platform_admin'));
...
values
  ('resident', 'Resident', 'Standard roommate access with personal dashboard visibility.'),
  ('house_manager', 'House Manager', 'Elevated access for operators managing amenities, bookings, and residents.'),
  ('platform_admin', 'Platform Admin', 'Portfolio-wide administrative controls and payment oversight.')
...
CREATE POLICY "Managers manage permissions" ON public.permission_table
  FOR ALL
  TO authenticated
  USING ((select role from public.profiles where id = auth.uid()) in ('house_manager','platform_admin'))
  WITH CHECK ((select role from public.profiles where id = auth.uid()) in ('house_manager','platform_admin'));
...
CREATE POLICY "Managers can review todos" ON public.todos FOR SELECT TO authenticated USING ((select role from public.profiles where id = auth.uid()) in ('house_manager','platform_admin'));

CREATE POLICY "Managers can moderate todos" ON public.todos FOR UPDATE TO authenticated USING ((select role from public.profiles where id = auth.uid()) in ('house_manager','platform_admin')) WITH CHECK ((select role from public.profiles where id = auth.uid()) in ('house_manager','platform_admin'));
